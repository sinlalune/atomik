...
- **[ADR-023 — The checker is held one patch ahead](./ADR-023-the-checker-is-held-one-patch-ahead.md)** · accepted · 2026-09-22
  Cairn 1.1 dropped the two link exemptions this repository's 0.2 checker carried — sample documents that portray another vault, and a journal frozen by its own header — and ruled them the adopter's own while offering the adopter no field to declare them in; `tools/cairn-check.mjs` is therefore held one patch ahead of its release, the fork is declared rather than hidden, and the ADR names the release feature and the one command that retire it.
