...
- **[ADR-020 — Protocol context weight is a first-order constraint](./ADR-020-protocol-context-weight.md)** · proposed · 2026-08-31
  Minimising what a reader must consume to execute the protocol becomes a maxim with a test — separate the must-do from the why, index the first and link the second — naming instruction parity as the reader-side twin of gate parity, declaring the portable/host/binding artefact boundary, and making the path record a folder that is born sliced rather than one that is rolled up later.
