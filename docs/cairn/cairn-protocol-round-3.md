...
| **C7: `ledger-size` Rule** | Listed `ledger-size` as an active rule in `cairn-check.mjs`. | **Was false when written; true since CP-OPS-002 S04 (2026-08-24).** It was a proposal with no code behind it. S04 implemented it as an ADVISORY, diff-scoped rule with a 10,000-token budget, and the table in §2.3 is regenerated from the live source. | `npm run cairn-check:test`<br>$\rightarrow$ `✔ an oversized path file in the diff advises rolling steps into history` |
