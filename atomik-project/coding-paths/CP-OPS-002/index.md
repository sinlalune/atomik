...
| Next action | **ADR-020 stages 3–5.** Stage 3's remaining half: retire `ledger-size` once `CP-MVP-008`, `CP-MVP-011` and `CP-MVP-012` migrate, and finish keying the checker on the shape. Then stage 4 (the artefact classification — the protocol page moves, the binding appendix is extracted, the host constitution leaves the entry chain) and stage 5 (`cairn-init` scaffolds the result, generation-aware retention included). S08 Part 2 also remains: an adversarial fixture for every blocking rule, one gate run in both invocation contexts asserting one verdict, and a generated conformance matrix |
