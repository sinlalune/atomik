...
- **S09e — done, and the closure restarts once more.** The integration pull request compared the branch with the trunk and found the S00 edits to twenty-six immutable session records; restored to their trunk blobs with a correction record. **Genesis debts:** the repair page's correction-record remedy has no predicate; `resolveBase` compares nothing on a non-path branch; the working-tree form of `record-integrity` reads status rather than content against the trunk; the binding still says `manual-git` while the trunk requires a pull request.
