...
- **Experiential gate:** the creator uses Atomik as the primary tool for learning notes for two consecutive weeks; every friction point is recorded as project files inside Atomik itself (notes, questions.md, log.md). This gate, not the functional tests alone, closes the path. *[Amended at owner review, 2026-07-06: the two-week block is replaced by the continuous dogfooding practice already running — see `../sessions/2026-07-06-cp-mvp-001-review.md`. Rationale: the gate's first real finding was that M2-without-sources has thin daily-use value; the flywheel produced and resolved 10+ owner frictions on day one; M3 (capture/sources) is what makes daily use real.]*
...
- [x] S12 Experiential gate: REFORMULATED and closed at owner review (2026-07-06). The block gate becomes the standing continuous practice (frictions as project files, fixed as micro-units — running since day one: creation inputs, tree collapse, soft breaks, AI accept semantics, panel dock, evidence-scope signal…). Priorities patched into `brainstorm/`; review record in `sessions/2026-07-06-cp-mvp-001-review.md`.
...
next action : none — PATH CLOSED at owner review 2026-07-06 (S12 reformulated:
              continuous dogfooding replaces the two-week block; review record in
              sessions/2026-07-06-cp-mvp-001-review.md). Successor: CP-MVP-002
              (capture sources + local speech baseline, M3) — see ACTIVE.md.
blockers    : none
```
