...
| Next action | Execute **S07g**: revise the canonical specification to v0.2 against the user-supplied nineteen-item review brief. Specification text only — no checker rules land in this step. Every item is either resolved in normative text or listed under *Deliberate non-goals* with a reason; every added requirement gains a conformance-matrix row with an honest reference-tools column; net concept count stays at or under 66 |
...
| Blockers | None. S07f is a completed remote checkpoint; S07g executes against it |
