...
Rule: exactly one active parent path at a time. Agents entering the repository follow `docs/bedrock/22_22-agent-handoff.md`; with no active path, do NOT start implementation — propose the next path from the register and the roadmap milestone (18 §M5), using the template in 24.
