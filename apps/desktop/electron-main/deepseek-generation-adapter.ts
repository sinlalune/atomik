...
/**
 * DeepSeek Direct Generation Adapter (CP-PROVIDERS S03)
 *
 * Direct first-party adapter for DeepSeek chat completions (deepseek-chat, deepseek-reasoner).
 * Uses DeepSeek API (OpenAI-compatible chat completions interface).
 */

const DEEPSEEK_CHAT_URL = 'https://api.deepseek.com/chat/completions'
const MAX_INPUT_TOKENS_ESTIMATE = 60_000
...
export function createDeepSeekGenerationAdapter(
  key: string,
  ...
  const fallbackModel =
    options.defaultModel ?? PROVIDER_CATALOG.deepseek.defaultModel
    ...
    generate: async (operation, context): Promise<GenerationResult> => {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          throw new GenerationError(
            'auth',
            `the DeepSeek key was rejected (HTTP ${response.status}) — check it in ☰ → Settings`
          )
        }
      ...
      const parsed =
        (await response.json().catch(() => ({}))) as DeepSeekChatResponse
