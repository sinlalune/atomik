...
/**
 * Google Gemini Direct Generation Adapter (CP-PROVIDERS S03)
 *
 * Direct first-party adapter for Google Gemini models (gemini-2.0-flash, gemini-1.5-pro, etc.)
...
export function createGoogleGenerationAdapter(
  ...
  return {
    ...
    generate: async (operation, context): Promise<GenerationResult> => {
      ...
      const parsed =
        (await response.json().catch(() => ({}))) as GoogleChatResponse
